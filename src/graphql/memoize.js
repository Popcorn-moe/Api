			...
			field.resolve = mem(field.resolve, {
				...
				normalizer([root, args, context]) {
					return JSON.stringify([
						root,
						args,
						context.user && context.user.unsafe_id
					]);
				}
			});
		}
...
Object.defineProperty(memoizee, "__profiler__", {
	value(conf) {
		...
		conf.on("set", () =>
			stats.increment(`backend.graphql.memoize.initial`, 1, [`resolver:${id}`])
		);
		conf.on("get", () =>
			stats.increment(`backend.graphql.memoize.cached`, 1, [`resolver:${id}`])
		);
	}
});
